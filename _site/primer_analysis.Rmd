...
```{r create_succes_file, message=FALSE, warning=FALSE, eval=FALSE}
...
for(file in BLAST_file){
  ...
  unknown <- merge(categories, unknown, by.x = "Primer", by.y = "reference")
  ...
  for(primer_file in files){
    if(file.info(primer_file)$size > 0){
      ...
      blast <- merge(categories, blast, by.x = "Primer", by.y = "reference")
...
saveRDS(primer_success, '../data/primer_success.RDS')
```

### Primers by Samples
```{r readin_success, message=FALSE, warning=FALSE, echo=FALSE}
primer_success <- readRDS('../data/primer_success.RDS')
```

```{r success_table, echo=FALSE, message=FALSE, warning=FALSE}
primer_success
```
...
### Overall for Primers
```{r primers, message=FALSE, warning=FALSE}
...
primers <- merge(classifications, primers, by.y = 'primer', by.x = 'Primer')
setorder(primers, -n_reads)
```

```{r primers_table, echo=FALSE, message=FALSE, warning=FALSE}
...
samples <- primer_success[, lapply(.SD, sum, na.rm=TRUE), by=sample, .SDcols=c("n_reads", "artifacts", "analyzed", "true_pos", "false_pos") ] 
setorder(samples, sample)
```

```{r samples_table, echo=FALSE, message=FALSE, warning=FALSE}
datatable(samples, options = list(
            columnDefs = list(
              list(
                targets = 1,
                width = '260px',
...
```{r sample_t_pos, message=FALSE, warning=FALSE}
...
read_count <- merge(classifications, read_count, by.y = 'primer', by.x = 'Primer')
```

```{r t_pos_counts, echo=FALSE, message=FALSE, warning=FALSE}
read_count
```
<br>
<br>
...
```{r overall_primer, warning=FALSE}
...
### All
```{r primer_artifact_graph, fig.width=12, fig.height=7, echo=FALSE, warning=FALSE}
...
ggplot(primers, aes_string(x="Primer", y="percent_artifacts")) + 
  ...
  scale_fill_manual(values = colors[c(2,5,4,3)], name = "Read Classification")
```

### Artifact
```{r _artifact_graph, fig.width=12, fig.height=7, echo=FALSE, warning=FALSE}
ggplot(primers, aes_string(x="Primer", y="percent_artifacts")) + 
  geom_bar(stat="identity", position="stack", 
...
### True-Positive
```{r _tp_graph, fig.width=12, fig.height=7, echo=FALSE, warning=FALSE}
ggplot(primers, aes_string(x="Primer", y="percent_artifacts")) + 
  geom_bar(stat="identity", position="stack", 
...
### False-Positive
```{r _fp_graph, fig.width=12, fig.height=7, echo=FALSE, warning=FALSE}
ggplot(primers, aes_string(x="Primer", y="percent_artifacts")) + 
  geom_bar(stat="identity", position="stack", 
...
### False-Negative
```{r _fn_graph, fig.width=12, fig.height=7, echo=FALSE, warning=FALSE}
ggplot(primers, aes_string(x="Primer", y="percent_artifacts")) + 
  geom_bar(stat="identity", position="stack", 
...
```{r example_artifacts, warning=FALSE, echo=FALSE}
fread('../data/example_artifacts.fa', header = FALSE, col.names = "SCI-Soil-D59-A R1 Reads")
```
...
```{r by_reads, warning=FALSE, echo=FALSE}
...
```{r by_read_graph, fig.width=6, fig.height=4, echo=FALSE, warning=FALSE}
...
```{r artifacts, echo=FALSE, warning=FALSE}
...
```{r artifacts_graph, fig.width=7, fig.height=5, echo=FALSE, warning=FALSE}
  ggplot(merge(metadata, samples,by.x='Sample Name', by.y='sample'), aes_string(x = "n_reads", y = "artifacts", color="Matrix")) +
   geom_point(size = 4, show.legend = T) + 
  geom_smooth(formula = y~x, method = "lm", se = FALSE, color = 'black' ) +
  scale_colour_manual(values = schuylR::create_palette(8)) + theme_bw() + 
  theme(axis.text.x = element_text(size = 10), 
        axis.text.y = element_text(size = 10),  
        axis.title.x = element_text(size = 12, face = "bold"), 
        axis.title.y = element_text(size = 12, face = "bold"), 
