...
read_counts <- data.table::dcast(primer_success, Primer ~ sample, value.var=c("true_pos"), fun = sum)
read_countss <- read_countss[c(which(rowSums(read_countss[,-1]) > 0)),]
read_countss <- merge(classifications, read_countss, by = 'Primer')
saveRDS(read_countss, '../data/primers/read_countss_from_primers.RDS')
```
...
data.table::set(primer_success, which(primer_success[["success_rate"]] == 0), "success_rate", NA)
primer_success[, experiment := sapply(strsplit(sample, "-"), "[[", 1)]
read_countss <- data.table::dcast(primer_success, Primer ~ sample, value.var=c("true_pos"), fun = sum)
read_countss <- read_countss[c(which(rowSums(read_countss[,-1]) > 0)),]
read_countss <- merge(classifications, read_countss, by = 'Primer')
```
