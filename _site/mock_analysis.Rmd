...
```{r primer_counts_out, echo=FALSE}
Primer_counts
```
...
```{r ARG_Family_counts_out, echo=FALSE}
ARG_Family_counts
```
...
`r round(mean(vegdist(t(dist_matrix[,-c(1,2)]))), 2)`
`r round(mean(as.matrix(vegdist(t(dist_matrix[,-c(1)])))[-1,1]), 2)`
