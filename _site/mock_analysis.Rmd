...
DARTE-QM found **`r length(unique(graph_data[Sample != "Theoretical"][ARG != "spike"][Abundance > 0]$ARG))`** (`r round(length(unique(graph_data[Sample != "Theoretical"][ARG != "spike"][Abundance > 0]$ARG))/length(unique(graph_data[Sample == "Theoretical"][ARG != "spike"]$ARG)) * 100, 1)`%) of the **`r length(unique(graph_data[Sample == "Theoretical"][ARG != "spike"]$ARG))`** primers in the Mock-Community pangenomes that were targeted.
...
spike_counts <- mock_observed_counts[ARG == 'spike']
```
