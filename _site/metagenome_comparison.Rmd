...
```{r profilecomp, echo=FALSE, message=FALSE, warning=FALSE}
...
### Diversity
```{r apha-div, echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=4}
index = "shannon"
colors = "default"
alpha <- compare_data
...
ggplot(alpha_data, aes(Matrix, Alpha)) + 
...
```{r missing, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
missing <- unique(compare_data[Tech != "DARTE-QM"]$ARG)[
  unique(compare_data[Tech != "DARTE-QM"]$ARG) %in% unique(compare_data[Tech == "DARTE-QM"]$ARG)
]
length(missing)
missing[(missing %in% classifications$ARG)]
```

  
## DARTE-QM Target ARGs {.tabset .tabset-pills}
### Profiles
```{r compare_targets, echo=FALSE, message=FALSE, warning=FALSE}
...
### Diversity
```{r apha-div_DARTE_Only, echo=FALSE, message=FALSE, warning=FALSE, fig.width=5, fig.height=4}
index = "shannon"
colors = "default"
alpha <- compare_data
alpha <- dcast(alpha, ARG ~ Sample, value.var = 'relative_abundance', fun.aggregate = sum)
