...
variant_counts <- data.table(Matrix = c("Mock", "Swine Manure-A", "Soil-A + Manure-A", "Soil-C", "Soil-B", "Effluent", "Swine Fecal", "Total"),
           Variants = c(length(unique(graph_data[Matrix == "Mock"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Swine Manure-A"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Soil-A + Manure-A"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Soil-C"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Soil-B"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Effluent"][value > 0]$Primer)),
                        length(unique(graph_data[Matrix == "Swine Fecal"][value > 0]$Primer)),
                        length(unique(graph_data[value > 0]$Primer))))
```

```{r ARG_counts_out, echo=FALSE}
datatable(variant_counts)
```
...
graph_data <- graph_data[!(ARG %in% c('16S'))]
graph_data <- merge(metadata[,c('ID', 'Sample', 'Matrix')], graph_data, by.x = 'ID', by.y = 'Sample_Name')
graph_data <- graph_data[, sum(value), by = c('Sample', 'Matrix', 'ARG_Family', 'ARG')]
...
graph_data <- rbindlist(list(
  cbind(graph_data[,c('Sample', 'ARG_Family', 'Matrix', 'V1')], Tech = 'DARTE-QM'), 
  cbind(SC_I[,c('Sample', 'ARG_Family', 'Matrix', 'V1')], Tech = 'Metagenome')), fill = TRUE)
...
classification_order <- setorder(graph_data[, lapply(.SD, sum, na.rm=TRUE), 
                                            by=ARG_Family, .SDcols=c("relative_abundance")], -relative_abundance)
classification_order <- classification_order[round(classification_order[[2]]*100 / 
                                                     length(unique(graph_data[Tech == "DARTE-QM"]$Sample)),1) >= 0.5,]
...
graph_data <- graph_data[, lapply(.SD, sum, na.rm=TRUE), by=c('Sample', "ARG_Family", 'Matrix', 'Tech'), 
...
profile <- ggplot(graph_data[Tech == 'DARTE-QM'], 
