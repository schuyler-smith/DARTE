...
```{r ARG_Families, message=FALSE, warning=FALSE}
...
```{r profile, echo=FALSE, warning=FALSE, fig.width=12, fig.height=6, cache=TRUE}
profile
```
<br>
<br>
...
```{r pca_calculation_non_normalized, message=FALSE, warning=FALSE}
treatment = 'Matrix'
x = 1
y = 2
method = 'bray'
...
## PERMANOVA
```{r permanova_code, message=FALSE, warning=FALSE}
graph_data <- read_counts[,-c(2,3)]
graph_data <- rarefy(data=graph_data[, c(1, which(colSums(graph_data[,-1])>5000)+1), with=FALSE], min(colSums(graph_data[,-1])))[,-1]
...
```{r bray_clustering, message=FALSE, warning=FALSE}
method = 'bray'
graph_data <- read_counts[,-c(2:3)]
graph_data <- rarefy(data=graph_data[, c(1, which(colSums(graph_data[,-1])>5000)+1), with=FALSE], min(colSums(graph_data[,-1])))[,-1]
distance_matrix <- vegan::vegdist(t(graph_data), method, na.rm = TRUE)
...
```{r bray_dendro, echo=FALSE, fig.width=12, fig.height=6, warning=FALSE, cache=TRUE}
sample_dendrogram
```
...
```{r jaccard_clustering, message=FALSE, warning=FALSE}
method = 'jaccard'
graph_data <- read_counts[,-c(2:3)]
graph_data <- rarefy(data=graph_data[, c(1, which(colSums(graph_data[,-1])>5000)+1), with=FALSE], min(colSums(graph_data[,-1])))[,-1]
distance_matrix <- vegan::vegdist(t(graph_data), method, na.rm = TRUE)
...
```{r jaccard_dendro, echo=FALSE, fig.width=12, fig.height=6, warning=FALSE, cache=TRUE}
sample_dendrogram
```
...
```{r euclidean_clustering, message=FALSE, warning=FALSE}
method = 'euclidean'
graph_data <- read_counts[,-c(2:3)]
graph_data <- rarefy(data=graph_data[, c(1, which(colSums(graph_data[,-1])>5000)+1), with=FALSE], min(colSums(graph_data[,-1])))[,-1]
distance_matrix <- vegan::vegdist(t(graph_data), method, na.rm = TRUE)
...
```{r euclidean_dendro, echo=FALSE, warning=FALSE, fig.width=12, fig.height=6, cache=TRUE}
sample_dendrogram
```
...
```{r gower_clustering, message=FALSE, warning=FALSE}
method = 'gower'
graph_data <- read_counts[,-c(2:3)]
graph_data <- rarefy(data=graph_data[, c(1, which(colSums(graph_data[,-1])>5000)+1), with=FALSE], min(colSums(graph_data[,-1])))[,-1]
distance_matrix <- vegan::vegdist(t(graph_data), method, na.rm = TRUE)
...
```{r gower_dendro, echo=FALSE, warning=FALSE, fig.width=12, fig.height=6, cache=TRUE}
sample_dendrogram
```
<br>
<br>
...
```{r k-means_analysis, message=FALSE, warning=FALSE}
method = 'bray'
x = 1
y = 2
graph_data <- read_counts[,-c(2:3)]
...
```{r opt_k_means, echo=FALSE, warning=FALSE, fig.width=6, fig.height=5, cache=TRUE}
opt_k_means
```
...
```{r K(3), message=FALSE, warning=FALSE}
k = 3
treatment = 'Matrix'
MDS <- cmdscale(distance_matrix, k = max(c(x,y)), eig = TRUE)
graph_data <- cbind(x = MDS$points[,x], y = MDS$points[,y])
...
```{r K(6), message=FALSE, warning=FALSE}
k = 6
cluster_colors <- schuylR::create_palette(k, colors = 'Dark2')
km <- kmeans(distance_matrix, centers = k, iter.max = 10, nstart = 20)$cluster
...
```{r K(6)_graph, echo=FALSE, warning=FALSE, fig.width=7, fig.height=5, cache=TRUE}
k6
```
