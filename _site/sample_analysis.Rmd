...
```{r setup, echo=FALSE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE, tidy = FALSE, size="small")
```
...
```{r profile, echo=FALSE, fig.width=12, fig.height=6, cache=TRUE}
profile
```
<br>
<br>
...
```{r pca_calculation_non_normalized}
treatment = 'Matrix'
x = 1
y = 2
method = 'bray'
...
```{r bray_dendro, echo=FALSE, fig.width=12, fig.height=6, cache=TRUE}
sample_dendrogram
```
...
```{r euclidean_dendro, echo=FALSE, fig.width=12, fig.height=6, cache=TRUE}
sample_dendrogram
```
...
```{r gower_dendro, fig.width=12, fig.height=6, cache=TRUE}
sample_dendrogram
```
<br>
<br>
